...
export default function createValidator(schema) {
  return values => {
    //i think this values is taken from the form redux
    ...
    const errors = result.error.details.reduce((all, cur) => {
      const allErrors = Object.assign({}, all); //an alternative way is written in javascript reducer file we can use spread operator here
